...
**AmneziaWG** - это **обфусцированный протокол** на основе WireGuard с улучшенными механизмами обхода Deep Packet Inspection (DPI) для обхода блокировок.

### История версий:
- **Amnezia Legacy (WireGuard)** - первая версия с базовой обфускацией
- **AmneziaWG 1.0** - улучшенная обфускация с дополнительными параметрами
- **AmneziaWG 2.0 (1.5)** - текущая версия с полным набором параметров обфускации

> ⚠️ **Важно:** Все версии Amnezia используют обфускацию. Legacy имеет базовую обфускацию, а AmneziaWG 2.0 - более продвинутую с дополнительными параметрами (S3, S4, I1-I5).
...
S1 = 52         # Init packet junk (15-150) - КРИТИЧЕН для обхода DPI!
S2 = 51         # Response packet junk (15-150) - должен отличаться от S1
S3 = 49         # Cookie reply packet junk (0-64) - для AmneziaWG 2.0
S4 = 10         # Data/Transport packet junk (0-20) - влияет на скорость

# Magic headers - имитация других протоколов (формат: min-max)
H1 = 1862172591-2144490164
H2 = 2147224357-2147329105
H3 = 2147339650-2147340356
H4 = 2147361801-2147383114

# Special patterns (специальные паттерны для дополнительной обфускации)
# Обычно пустые или содержат специальные значения
I1 = 
I2 = 
I3 = 
I4 = 
...
def create_vpn_link(private_key, client_ip, server_pubkey, preshared_key, 
                    ...
                    jc=4, jmin=10, jmax=50, s1=52, s2=51, s3=49, s4=10,
                    h1="1862172591-2144490164", h2="2147224357-2147329105", 
                    h3="2147339650-2147340356", h4="2147361801-2147383114",
                    i1="", i2="", i3="", i4="", i5=""):
    """
...
| Параметр | Описание | Рекомендуемое значение | Версия |
|----------|----------|------------------------|--------|
| **Jc** | Количество junk-пакетов | 3-7 (4-5 рекомендуется) | Все |
| **Jmin** | Минимальный размер junk | 10-50 (10 рекомендуется) | Все |
| **Jmax** | Максимальный размер junk | 50-1000 (50 рекомендуется) | Все |
| **S1** | Init packet junk | 15-150 (**КРИТИЧЕН!** 50-85 рекомендуется) | Все |
| **S2** | Response packet junk | 15-150 (отличается от S1, 50-140 рекомендуется) | Все |
| **S3** | Cookie reply packet junk | 0-64 (28-50 рекомендуется) | AWG 2.0 |
| **S4** | Data/Transport packet junk | 0-20 (10-15 рекомендуется, влияет на скорость) | AWG 2.0 |
| **H1-H4** | Magic headers | Диапазоны чисел (формат: min-max) | Все |
| **I1-I5** | Special patterns | Обычно пустые или специальные значения | AWG 2.0 |

**⚠️ ВАЖНО:** 
- Параметры обфускации должны быть **ИДЕНТИЧНЫ** на сервере и всех клиентах!
- S3, S4, I1-I5 доступны только в AmneziaWG 2.0 (1.5)
- H1-H4 указываются в формате диапазона: `min-max` (например, `1862172591-2144490164`)
